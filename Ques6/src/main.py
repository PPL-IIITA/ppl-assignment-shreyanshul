...
exCouples = []
def formNewCouple(couples, exCoup):
		'Form New Couples'
		print
		print'There is breakup between couples, and they are following :'
		print "\n".join(['Earlier ' + i[1].name+ ' was comitted with ' + i[0].name for i in exCoup])
		...
		for i in exCoup:
				b = i[0]
				for g in g1:
					if( g.rel_stat == 'single' and g != i[1] ):
						couples += [(g,b)]
						logger(g.name + " is commited with " + b.name)
						b.rel_stat = 'comitted'
						g.rel_stat = 'comitted'
		...
		print "\n".join(['Now ' + i[0].name + ' is comitted with ' + i[1].name for i in couples])

def breakUp( exCouples, gfbf ):
	'Breakup of Couples'
	days = randint(1,16)
	limit = randint( 200, 501 )
	for i in range (1, days+1):
			giftingHappCal( gfbf, gift, detail)
			for j in detail:
					if j[3] < limit:
							logger(j[1].name + " has broken up with " + j[0].name)
							exCouples += [ (j[0],j[1]) ]
							...
							for k in gfbf:
								if k[0] == j[1] and k[1] == j[0] :
									...
									break
							detail.remove( j )
			formNewCouple( gfbf, exCouples )
		
breakUp( exCouples, gfbf )
